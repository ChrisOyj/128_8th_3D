...
CNSAttack_3* CNSAttack_3::Create()
{
    CNSAttack_3* pInstance = new CNSAttack_3();

    if (FAILED(pInstance->Initialize()))
    {
        Call_MsgBox(L"Failed to Initialize : CNSAttack_3");
...
void CNSAttack_3::OnCollisionEnter(CGameObject* pOtherObj, const _uint& eColType, _float4 vColPos)
{
    __super::OnCollisionEnter(pOtherObj, eColType, vColPos);
}

HRESULT CNSAttack_3::Initialize()
{
    __super::Initialize();

    m_eAnimType = ANIM_ATTACK_NINZASWORD;
    m_iAnimIndex = 31;
    m_eStateType = STATE_NSATTACK_3_PLAYER;
    ...
    m_fAnimSpeed = 2.5f;
    m_fInterPolationTime = 0.01f;

    m_iBoneChangeFrame = 98;
    m_iAttackExecuteFrame = 13;
    m_iSecondAttackExecuteFrame = 32;
    m_iStateChangableFrame = 70;
...
void CNSAttack_3::Enter(CUnit* pOwner, CAnimator* pAnimator)
{
...
STATE_TYPE CNSAttack_3::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
    return __super::Tick(pOwner, pAnimator);
}

void CNSAttack_3::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
    __super::Exit(pOwner, pAnimator);
}

STATE_TYPE CNSAttack_3::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
...
void CNSAttack_3::OnExecute(CUnit* pOwner, CAnimator* pAnimator)
{
