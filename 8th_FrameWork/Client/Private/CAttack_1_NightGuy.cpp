...
#include "Functor.h"
CAttack_1_NightGuy::CAttack_1_NightGuy()
{
}
...
CAttack_1_NightGuy* CAttack_1_NightGuy::Create()
{
	CAttack_1_NightGuy* pInstance = new CAttack_1_NightGuy();

	if (FAILED(pInstance->Initialize()))
	{
		Call_MsgBox(L"Failed to Initialize : CAttack_1_NightGuy");
...
HRESULT CAttack_1_NightGuy::Initialize()
{
	m_eAnimType = ANIM_EIGHTGATES;
	m_iAnimIndex = 8;
	m_eStateType = STATE_ATTACK_1_NIGHTGUY;

	m_vecAdjState.push_back(STATE_ATTACK_2_NIGHTGUY);
...
void CAttack_1_NightGuy::Enter(CUnit* pOwner, CAnimator* pAnimator)
{
...
STATE_TYPE CAttack_1_NightGuy::Tick(CUnit* pOwner, CAnimator* pAnimator)
{
...
STATE_TYPE CAttack_1_NightGuy::Check_Condition(CUnit* pOwner, CAnimator* pAnimator)
{
...
void CAttack_1_NightGuy::OnExecute(CUnit* pOwner, CAnimator* pAnimator)
{
	...
	vFireTargetDir.Normalize();
	if (pOwner->Get_PhysicsCom()->Get_Physics().bAir)
	{
		pOwner->Get_PhysicsCom()->Set_Jump(4.f);
		pOwner->Get_PhysicsCom()->Get_Physics().fGravity = 9.8f;
		pOwner->Get_PhysicsCom()->Set_Speed(5.f);
		pOwner->Get_PhysicsCom()->Set_Dir(vFireTargetDir * -1.f);
	}
	...
	_float4 vTargetPos = vPos + vFireTargetDir * 200.f;
...
void CAttack_1_NightGuy::Exit(CUnit* pOwner, CAnimator* pAnimator)
{
